...
public class StrongPrimeGenerator {
	...
	public static BigInteger generateStrongPrime(int bitLength) {
		...
		// int i = 0;
		// do {
		// strongPrimeCandidate = p.add(BigInteger.valueOf(2 *
		//// i).multiply(p1).multiply(p2));
		// i++;
		...
		SecureRandom random = new SecureRandom();
        BigInteger s = Math.randomPrime(bitLength / 2);
        BigInteger t = Math.randomPrime(bitLength / 2);
	...
	// }
	// boolean found = false;
	// for (BigInteger j = BigInteger.ZERO; j.compareTo(q) < 0; j =
	...
	// // Step 3
	// BigInteger p0 =
	// BigInteger.valueOf(2).multiply(s.modPow(r.subtract(BigInteger.valueOf(2)),
	// r)).multiply(s)
	...
	// BigInteger temp =
	// p0.add(BigInteger.valueOf(2).multiply(BigInteger.valueOf(j)).multiply(r).multiply(s));
	// if (temp.isProbablePrime(100)) {
	// p = temp;
	// break;
	// }
